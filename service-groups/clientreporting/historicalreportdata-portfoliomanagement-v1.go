...
func GetAsyncPortfolioManagement(clientkey string, fromdate string, todate string, params GetAsyncPortfolioManagementParams) ([]byte, error) {
	resp, err := saxo.GetClient().HttpClient.Get("https://gateway.saxobank.com/sim/openapi/cr/v1/reports/Portfolio/{ClientKey}/{Fromdate}/{ToDate}/?AccountGroupKey={AccountGroupKey}&AccountKey={AccountKey}")
	if err != nil {
		return nil, err
	}
...
func GetMeAsyncPortfolioManagement(fromdate string, todate string, params GetMeAsyncPortfolioManagementParams) ([]byte, error) {
	resp, err := saxo.GetClient().HttpClient.Get("https://gateway.saxobank.com/sim/openapi/cr/v1/reports/Portfolio/me/{Fromdate}/{ToDate}/?AccountGroupKey={AccountGroupKey}&AccountKey={AccountKey}")
	if err != nil {
		return nil, err
	}
	return resp.Bytes(), nil
